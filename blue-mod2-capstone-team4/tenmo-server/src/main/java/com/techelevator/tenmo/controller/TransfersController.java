...
public class TransfersController {
    ...
    @RequestMapping(path = "account/myTransfers/{id}", method = RequestMethod.GET)
    public List<Transfer> allTransfers(@PathVariable long id) {
        List<Transfer> transfers = transfersDao.allMyTransfers(id);
        return transfers;
    }
    @RequestMapping(path = "account/transfer/{myId}/{transferId}", method = RequestMethod.GET)
    public Transfer getTransfer(@PathVariable long myId, @PathVariable long transferId) {
        return transfersDao.transferById(myId, transferId);
    }
